...
int sm3_digest_with_preprocess(const unsigned char* message,
  ...
  if (error_code != 0) {
    ELOG_ERROR("Compute SM3 digest of leading data Z failed");
    return COMPUTE_SM3_DIGEST_FAIL;
  }
...
TeeErrorCode Sm3Crypto::Hash(const std::string& data, std::string* hash) {
  char hash_buf[kSm3HashSize] = {0};
  unsigned int hash_len = 0;
  const EVP_MD* md = EVP_sm3();
  EVP_MD_CTX* md_ctx = EVP_MD_CTX_new();
  if (!md_ctx) {
    ELOG_ERROR("Fail to create sm3 ctx");
    return TEE_ERROR_CRYPTO_SM3;
  }
  ...
  if (hash_len != kSm3HashSize) {
    ELOG_ERROR("Unexpected hash result length");
    return TEE_ERROR_CRYPTO_SM3;
  }
...
TeeErrorCode Sm3Crypto::GetHashSize() {
  return kSm3HashSize;
}
